...
class UsersTableSeeder extends Seeder
{
    public function run(): void
    {
        ...
        (new WalletService)->deposit($senior, 5 * 500_000_000, TransactionName::CapitalDeposit);

        $owner = $this->createUser(UserType::Owner, 'Owner', 'O3456454', '09876556665', $senior->id, 'vH4Hu34');
        (new WalletService)->transfer($senior, $owner, 5 * 1000000.00, TransactionName::CreditTransfer);
        ...
        (new WalletService)->transfer($agent, $player, 100000.00, TransactionName::CreditTransfer);
        ...
        (new WalletService)->transfer($agent, $player1, 10000.00, TransactionName::CreditTransfer);
        $player2 = $this->createUser(UserType::Player, 'Player3', 'Player003', '09111111113', $agent->id);
        (new WalletService)->transfer($agent, $player2, 10000.00, TransactionName::CreditTransfer);
        $player3 = $this->createUser(UserType::Player, 'Player4', 'Player004', '09111111114', $agent->id);
        (new WalletService)->transfer($agent, $player3, 10000.00, TransactionName::CreditTransfer);
        $player4 = $this->createUser(UserType::Player, 'Player5', 'Player005', '09111111115', $agent->id);
        (new WalletService)->transfer($agent, $player4, 10000.00, TransactionName::CreditTransfer);
    }
