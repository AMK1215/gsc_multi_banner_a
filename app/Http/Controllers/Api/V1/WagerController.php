<?php

namespace App\Http\Controllers\Api\V1;

use App\Http\Controllers\Controller;
use App\Http\Resources\SeamlessTransactionResource;
use App\Models\User;
use App\Traits\HttpResponses;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class WagerController extends Controller
{
    use HttpResponses;

    public function index(Request $request)
    {
        $type = $request->get('type');

        [$from, $to] = match ($type) {
            'yesterday' => [now()->subDay()->startOfDay(), now()->subDay()->endOfDay()],
            'this_week' => [now()->startOfWeek(), now()->endOfWeek()],
            'last_week' => [now()->subWeek()->startOfWeek(), now()->subWeek()->endOfWeek()],
            default => [now()->startOfDay(), now()],
        };

        $user = auth()->user();
        $transactions = DB::table('users')
            ->leftJoin('reports', 'users.user_name', '=', 'reports.member_name')
            ->leftJoin('products', 'products.code', '=', 'reports.product_code')
            ->select(
                'users.user_name',
                'products.name as provider_name',
                'products.code',
                DB::raw('MIN(reports.created_at) as from_date'),
                DB::raw('MAX(reports.created_at) as to_date'),
                DB::raw('SUM(reports.payout_amount) as total_payout_amount'),
                DB::raw('SUM(reports.bet_amount) as total_bet_amount'),
                DB::raw('count(reports.product_code) as total_count')
            )
            ->where('users.id', $user->id)
            ->where('reports.status', '101')
            ->whereBetween('reports.created_at', [$from, $to])
            ->groupBy('users.user_name', 'users.name', 'products.name' ,'products.code')
            ->get();
        
        return $this->success(SeamlessTransactionResource::collection($transactions));
    }

    private function makeJoinTable()
    {
        $query = User::query();
        $query->join('results', 'results.user_id', '=', 'users.id');

        return $query;
    }
}
