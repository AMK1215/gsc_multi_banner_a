    ...
    public function createWagerTransactions(array $betBatch, SeamlessEvent $event)
{
        ...
        try {
            DB::transaction(function () use ($betBatch, $userId, $seamlessEventId) {
                ...
                foreach ($betBatch as $transaction) {
                    ...
                    // Check for existing wager without locking (since isolation is handled outside)
    ...
    // Return the seamless transactions (assuming this is the expected return value)
    return SeamlessTransaction::whereIn('seamless_transaction_id', array_column($betBatch, 'TransactionID'))->get();
}
